...
**Status: runnable preview, not yet MVP.** Ten bidding hands and eight play
...
**Current focus:** finish EC-043's two remaining reviewed scenarios, content-only.
EC-048's auction-bound validation is merged.

Readiness is calculated from [fixed weighted gates](docs/MVP_STATUS.md#fixed-weighted-readiness-gates):
**15 + 15 + 15 = 45%**. The CI badge reports main's formatting, analysis, tests,
scenario validation and web build separately.

## Seven checkpoints to MVP

1. **Finish EC-043** — two remaining reviewed exact-bid-protection scenarios.
